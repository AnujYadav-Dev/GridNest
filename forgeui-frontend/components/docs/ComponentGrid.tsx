  ...
  return (
    <div>
      ...
      <div className="mb-8 flex flex-col sm:flex-row gap-4">
        {/* Category filter */}
        <div className="flex items-center gap-1 p-1 rounded-[var(--gridnest-radius-md)] bg-[var(--gridnest-surface)] border border-[var(--gridnest-border)]">
          {CATEGORIES.map((cat) => (
            <button
              key={cat.value}
              id={`filter-${cat.value}`}
              onClick={() => setActiveCategory(cat.value)}
              className={`relative px-3 py-1.5 text-xs rounded-[var(--gridnest-radius-sm)] transition-colors ${
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-1.5 rounded-[var(--gridnest-radius-md)] border border-[var(--gridnest-border)] bg-[var(--gridnest-surface)] text-xs text-[var(--gridnest-text-muted)] hover:border-[var(--gridnest-border-hover)] transition-colors sm:ml-auto"
        >
          <Search size={12} />
          Search components...
